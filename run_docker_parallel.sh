...
echo "NOTE: This script assumes we are running on Matt's system with 8 graphics cards. If there aren't 8 graphics cards, the code will fail bc there isn't enough memory to load 2 nets on a single GPU."
...
sudo nvidia-docker run -it \
    ...
    gpu /ML_OD_Benchmarking/detectron_scripts/benchmark_parallel.sh
